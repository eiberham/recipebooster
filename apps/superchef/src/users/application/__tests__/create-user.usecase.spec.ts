...
describe('CreateUserUseCase', () => {
    ...
    beforeAll(async () => {
        ...
        const { unit: producer } = await TestBed.solitary(RabbitMQProducer)
            .compile();

        (createUserUseCase as any).rabbitMQProducer = producer;
        
        jest.spyOn((createUserUseCase as any).rabbitMQProducer, 'sendToQueue').mockResolvedValue(Promise.resolve());
        jest.spyOn(createUserUseCase, 'hashPassword').mockResolvedValue('hashedpassword');
    });
    ...
    it('should create a user', async () => {
        ...
        expect((createUserUseCase as any).rabbitMQProducer.sendToQueue).toHaveBeenCalledWith('email_queue', {
            id: user.id,
            email: user.email,
            name: user.name,
